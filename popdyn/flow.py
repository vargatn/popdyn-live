...
class CellSpace(object):
    def __init__(self, initial_maps, layers, rules):
        ...
        self.padded_canvas = pad(self.canvas)
    ...
    def _make_indexes(self):
        ishape = self.shape_canvas[1:]  # only the map part of the stack
        self.imap = np.arange(ishape[0] * ishape[1]).reshape(ishape)
        ishape = self.shape_canvas_padded[1:]  # only the map part of the stack
        self.imap_padded = np.arange(ishape[0] * ishape[1]).reshape(ishape)
        self.imap_center = center_cut(self.imap_padded)
        self.imap_shifted = np.array([center_cut(tmp) for tmp in calc_shifted(self.imap_padded)])
    ...
    def shift(self):
        ...
        for i, ii in enumerate(self.imap_shifted):
            for j in np.arange(self.canvas.shape[0]):
